...
const reScript = /\.(tsx|ts|js|jsx|mjs)$/;
...
const config = {
  ...
  resolve: {
    // alias: {
    //   'dt-components$': resolvePath(
    //     'node_modules/dt-components/lib/es/components/index.js'
    //   )
    // },
    plugins: [
...
if (process.argv.includes('--cypress')) {
  const babelLoaderConfig = config.module.rules.find(moduleRuleObj => {
    return moduleRuleObj.loader === 'babel-loader';
  });
  if (babelLoaderConfig) {
    babelLoaderConfig.options.plugins.push('transform-class-properties');
    babelLoaderConfig.options.plugins.push('babel-plugin-istanbul');
  }
}
//
...
const clientConfig = {
  ...
  plugins: [
    new InjectManifest({
      swDest: 'sw.js',
      swSrc: resolvePath('src/sw-template.js'),
    ...
    new ReactLoadableWebpackPlugin({
